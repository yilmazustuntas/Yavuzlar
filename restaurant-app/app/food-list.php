...
<body>
    <div class="centerDiv">
        <div class="centerDiv">
            ...
            <a href="./add-food.php" class="b<?php echo $_SESSION['role']; ?>"><button style="border-radius: 15px; background-color: #ffc400; color: black;">Yemek Ekle</button></a>
        </div>
        <?php if (empty($datas)) {
            echo "<p class='searchbox'>Firmaya ait hiçbir yemek bulunamadı.</p>";
        } else { ?>
            <div class="searchbox">
                <input style="border-radius: 8px; width: 200px;" type="search" id="searchbox" placeholder="Yemek Ara" />
                <div>
                    ...
                    <tbody>
                        <?php foreach ($datas as $data): if ($data['food_name']) { ?>
                                <tr class="t<?php echo $_SESSION['role']; ?> dataElement dataTable" is-banned="<?php echo $data['food_deleted_at'] ? 'true' : 'false'; ?>">
                                    ...
                                    <td>
                                        <a href="update-food.php?f_id=<?php echo $data['food_id']; ?>"><button style="border-radius: 15px;">Yemeği Güncelle</button></a>
                                    </td>
                                    <?php if ($data['food_deleted_at']): ?>
                                        <td>
                                            <form action="./scripts/readd-food.php" method="post">
                                                ...
                                                <button style="border-radius: 15px; background-color: #ffc400; color: black;" type="submit">+</button>
                                            </form>
                                        </td>
                                    <?php endif ?>
                                    <td>
        ...
        <?php } ?>
        <div class="centerDiv">
            <a href="index.php" class="clearText b<?php echo $_SESSION['role']; ?>"><button style="border-radius: 15px; background-color: #ffc400; color: black;" type="button">Ana Sayfa</button>
            </a>
        </div>
    </div>
