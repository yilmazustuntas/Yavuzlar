...
<body>
    <div class="centerDiv">
        ...
        <?php if (empty($datas)) {
            ...
            <div class="searchbox">
                <input style="border-radius: 8px; width: 200px;" type="search" id="searchbox" placeholder="Müşteri Ara" />
                <div>
                ...
                <tbody>
                    <?php foreach ($uniqueDatas as $data): ?>
                        <tr class="dataElement dataTable t<?php echo $_SESSION['role']; ?>" is-banned="<?php echo $data['user_deleted_at'] ? 'true' : 'false'; ?>">
                            ...
                            <td>
                                <p> <?php echo !is_null($data['order_status']) ? "<a href='user-orders.php?u_id=" . $data['user_id'] . "'><button style='border-radius: 15px;'>Siparişler</button></a>" : "Mevcut sipariş yok"; ?></p>
                            </td>
                            <td>
                                <p> <?php echo $data['user_created_at']; ?> </p>
                            </td>
                            <td>
                                <p> <?php echo $data['user_deleted_at'] ? 'Banlı (' . $data['user_deleted_at'] . ")" : '-'; ?> </p>
                            </td>
                            <td>
                                <div>
                                    <form action="./scripts/make-employee-query.php" method="post">
                                        <input type="hidden" name="user_id" value="<?php echo $data['user_id']; ?>" />
                                        <select style="border-radius: 10px; height: 30px;" name="company_id" id="company_id">
                                            <option value="" disabled selected>Seçiniz</option>
                                            <?php foreach ($companies as $company): ?>
                                                <option style="border-radius: 10px;" value="<?php echo $company['id']; ?>"><?php echo $company['name']; ?></option>
                                            <?php endforeach ?>
                                        </select>
    ...
    <div class="centerDiv">
        <a href="index.php" class="searchbox cleanText" style="margin-top:1.5rem;"><button style="border-radius: 15px;">Ana Sayfa</button></a>
    </div>
